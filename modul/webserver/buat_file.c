...
#ifdef PAKAI_PM
	else if (pmx[no].tipe==0 || pmx[no].tipe==1|| pmx[no].tipe==2 || pmx[no].tipe==3 || pmx[no].tipe==4 || pmx[no].tipe==5 ) 
	{		// Power Meter
		for (i=0; i< PER_SUMBER; i++)	{
			sprintf(head_buf, "<tr>\n<td>Kanal %d</td>\n<td align=\"right\">%.2f</td>\n", (no*PER_SUMBER+i+1), data_f[no*PER_SUMBER+i]);
			strcat(tot_buf, head_buf);
		...
		if (flag==3) {			// konfig modul sumber 
				...
				for (i=0; i<JML_SUMBER; i++) {					
					...
					sprintf(head_buf, "<tr><form action=\"setting.html\" name=\"mF%d\">\r\n" \
						...
						p_sbrw[i].stack, \
						p_sbrw[i].alamat, \
						(p_sbrw[i].tipe==0?"checked":" "), (p_sbrw[i].tipe==1?"checked":" "), (p_sbrw[i].tipe==2?"checked":" "),  (p_sbrw[i].tipe==3?"checked":" "),(p_sbrw[i].tipe==4?"checked":" "), (p_sbrw[i].tipe==5?"checked":" "),(p_sbrw[i].tipe==100?"checked":" "), \
